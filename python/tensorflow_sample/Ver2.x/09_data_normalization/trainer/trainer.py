...
class TrainerResNet(Trainer):
	# --- コンストラクタ ---
	def __init__(self, input_shape, classes, output_dir=None, model_type='custom', optimizer='adam', initializer='glorot_uniform'):
		# --- Residual Block ---
		...
		def _load_model(input_shape, classes, stack_fn, initializer='glorot_uniform'):
		...
		def _load_model_resnet50(input_shape, classes, initializer='glorot_uniform', dbg_mode=1):
			...
			if (dbg_mode == 0):
				...
				model = _load_model(input_shape, classes, stack_fn, initializer=initializer)
		...
		if (model_type == 'custom'):
			...
			self.model = _load_model(input_shape, classes, stack_fn, initializer=initializer)
